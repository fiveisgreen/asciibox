...
def add_comments(text_box, comment_mark, True_short__False_long = True): #WAD
    ...
    if True_short__False_long:
        for i in range(len(text_box)):# for line in text_box:
...
def put_A_next_to_B(A,B,space_string):
    ...
    if lA < lB:
        ...
        if lA > 0:
            wA = len(A[0])
        for i in range(minlen,lB):
            out.append((" "*wA)+space_string+B[i])
    elif lB < lA:
        wB = 0
        if lB > 0:
            wB = len(B[0])
        for i in range(minlen,lA):
...
def space_pad(text, n_left=1, n_right=1, fillchar = ' '): #wad
    ...
    for i in range(len(text)):# for line in text:
...
def build_textbox_signpost(text, center_text,min_struct_width, min_struct_length): #signpost
    ...
    #       | |    This is a #signpost    | |   
    #       | |      Let it be known      | |   
    #       | | that it will say things,  | |   
    #       | |   It looks best in Menlo  | |   
    #     __| |___________________________| |__ 
    #    (__   ___________________________   __)
    #       !_!                           !_!
    box_obj = simple_box()
...
def build_textbox_hearye(text,center_text,min_struct_width, min_struct_length): #hearye
    #  ____________________________________    
    # / \                                  \   
    #|   |                                 |   
    # \_ |                                 |   
...
# --> fix the #moo box
# --> make bash and csh echo modes. Wrap art in echo "...". Bash needs `=>\`. csh can't handle ` in echo.
...
#          ,r  _____!L___,M Dr. Anthony Barker Lsr--x_"^^`" qP     9      J      `M
